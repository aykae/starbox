...
def setup():
    global img, filenames, fileIndex
    matrix.start()
    filenames = ["img.txt", "img1.txt", "img2.txt", "img3.txt", "img4.txt"]


def loop():
    global ani, frame, frameCount, scolors, filenames, fileIndex

    matrix.clear()
    loadImg(filenames[fileIndex])
    fileIndex = (fileIndex + 1) % len(filenames)
