...
def loop():
    ...
    if starCount < MAX_STARS:
        ...
        if nextStar not in stars.keys() and (time.time_ns() - prevStarTime) >= delay:
            prevStarTime = time.time_ns()
            delay = (10**9) * random.randint(0, 1) # 1-2 seconds
    ...
    for star in stars.keys():
        if stars[star][0] == 1: #brightenining
            if stars[star][1] >= MAX_BRIGHTNESS:
                stars[star][0] = 2
                dimDelay = (10**9) * random.randint(3, 10)
